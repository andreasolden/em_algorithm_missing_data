...
em_alg=function(y1,x,niter=100,conv=1e-5,verbose=TRUE) #y1=caught, x=matrix(10*rnorm(px*n),n,px)
{
        ...
        emoutput<-list(em_a=em_a,em_b=em_b,yy=yy,iter=iter, binomial_coeff=binomial_coeff)
        return(emoutput)
}
...
datagen <- function(n, alfas, betas, xmeans, xsd, xcor){
        ...
        #Use mvrnorm to draw correlated data
        x=as.matrix(mvrnorm(n=n, mu=xmeans, Sigma=covar_matrix))
        
        thtrue=c(alfas,betas)
        p=matrix(1,n,3) # Below it produces infinity b/c e/100 etc is very large. The Infs come together???
        for(k in 1:2) p[,k+1]=exp(alfas[k]+x%*%betas) # for b [,2] and [,3] are equal because of b
        p=p/apply(p,1,sum) # force sum to one
        y=matrix(0,n,3) # 
        for(i in 1:n) y[i,]=rmultinom(1,1,prob=p[i,]) # draw based on 
        y1=y[,3] # OBSERVED FRAUDULANT CLAIMS
        datagenoutput <- list(p=p,y=y,y1=y1,thtrue=thtrue, x=x)
...
dfs=as.data.frame( 
        ...
        #Run EM algorithm. Note binomial model embedded in EM
        em_res = em_alg(data$y1,data$x)
                
        #Run a trinomial model on perfectly observed data
        thpi=data$thtrue*2 #Need some starting values.
        mlogit=optim(thpi,logl,x=data$x,y=data$y) 
