#Andreas SEM implementation august 2019
...
dm=function(thopt,th0,y1,x,niter=100,verbose=FALSE)
{
        d=length(thopt)
        r=matrix(0,d,d)
        for(iter in 1:niter)
...
sem=function(thopt,th0,y1,x,niter=100,verbose=FALSE)
{
...
#### SEM uten oppskalering
        n=1000
        px=2
        #px=1
        x = matrix(cbind( rnorm(n, 50, 12.3), rnorm(n, 10, 1.8) ), n, px )
